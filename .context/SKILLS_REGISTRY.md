...
| `system-manager` | Opus 4.7 / high (plan mode, read-only) | **CEO agent.** Monthly: reconciles every division's report against MISSION, finds cross-division gaps/misalignments, owns the org structure. Proposes + directs; never executes. See `.context/org/ORG.md`. | Mgmt |
| `ops-manager` | Opus 4.7 / high (plan mode, read-only) | **Platform Ops department manager.** Weekly: evaluates its division's health/deploy/DR state → report with prioritized recommendations. Proposes; never executes. Exemplar for the manager-hierarchy pattern. | Mgmt |
...
| `delivery-manager` | Opus 4.7 / high (plan mode, read-only) | **Delivery department manager.** Weekly: evaluates the project lifecycle (create→host→deploy→verify→update) across its roster + delivery-contract adoption + dev-repo coherence → report with prioritized recommendations. Proposes; never executes. | Mgmt |
| `knowledge-manager` | Opus 4.7 / high (plan mode, read-only) | **Knowledge department manager.** Weekly: evaluates research/ideas cadence, content dedup, off-site backup state → report with prioritized recommendations. Proposes; never executes. | Mgmt |
| `atlas-manager` | Opus 4.7 / high (plan mode, read-only) | **Atlas department manager.** Weekly: evaluates the atlas product sub-org (reports/scout/brief/portfolio/deploys) + its standing standards (single-writer, `ANTHROPIC_API_KEY` boundary) → report with prioritized recommendations. Proposes; never executes. | Mgmt |
