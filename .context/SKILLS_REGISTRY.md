...
| `atlas-evaluate` | Sonnet 4.6 / medium (→ Opus 4.7 / high on failure) | Weekly living-loop evaluator (atlas dev clone): adopts the repo's project-evaluator charter, scores the rubric with evidence, triages the `data_gaps` ledger (filed → triaged/rejected; free-only policy binding), re-routes `evaluation/BACKLOG.md`, commits + pushes. Judges and routes; never builds or deploys. Schedule: Mon 11:00; staged from atlas `integrations/ai-server/` | — |
| `atlas-gap-scout` | Sonnet 4.6 / medium (→ Opus 4.7 / high on failure) | Weekly gap-scout (atlas dev clone): adopts the pipeline-scout charter, takes the top triaged data gap, researches a FREE source, runs the live probe from the Mini, writes the engineer-ready spec (`knowledge/<sector>/pipelines.md`), marks the gap SPECCED, pushes. Ceiling FEED_SPECCED; never builds. Schedule: Wed 11:00; staged from atlas `integrations/ai-server/` | — |
| `atlas-refresh-knowledge` | Sonnet 4.6 / medium (→ Opus 4.7 / high on failure) | Monthly knowledge maintenance (atlas dev clone): curator condensation of `knowledge/*/CLAUDE.md` to 150-line budgets, re-verification of stale (>90d) load-bearing claims with fresh sources, `gaps-sync` ledger reconciliation, glossary guard, pushes. Schedule: 1st 11:30; staged from atlas `integrations/ai-server/` | — |
