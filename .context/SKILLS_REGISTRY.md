...
| `server-deploy` | Sonnet 4.6 / low (→ Opus 4.7 / high on failure) | Deploy the ai-server itself: sync learnings, ff-only pull, migrate, pytest gate (red = no restart), restart web/bot + detached runner restart. Trigger: `/task deploy server` | P0 |
