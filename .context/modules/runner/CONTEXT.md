...
- `review.run_code_review(parent_job_id, diff, cwd)` — sub-agent that evaluates diffs post-session for code-touching skills at a FIXED reviewer (opus-4-7/high; not skill-chosen — a skill can't downgrade the reviewer of its own diffs). `review.head_commit_epoch(cwd)` + `review.is_stale_head(head_epoch, job_started_epoch)` — pure wrong-diff guard: HEAD predating the job ⇒ `_maybe_review` skips with `post_review_skipped` instead of grading someone else's commit. NOTE (2026-08-05): post_review is a SECOND belt that only FLAGS — it stamps `review_outcome` (job stays `completed`) and on blocker/error emits a `post_review_flagged` audit event + a `review_flagged` task DM; it does NOT park or gate anything (the merge gate is each skill's in-session `code-review` subagent before the push; the review here runs after the code is already pushed). The post-session Job refetch (`_refetch_job`, both success and failure paths) is what makes resolved_skill visible — the review had never fired before it (0/516); the failure-path refetch also fixes level-0 escalation reading a stale skill.
...
- `workspaces.resolve_isolation(skill_iso, payload_iso)` — pure: effective isolation tier (`none | workspace | host`). HARDENED 2026-08-31 (EVALUATION_2026-08-30 F1): payload may only TIGHTEN (anything → `workspace`); `host`/`none` relaxation from a payload is ignored with a warning; unknown tiers fail closed to `workspace` (was `none`). The retired `container` value still maps to `workspace`. `session.run_session` additionally forces generic no-skill tasks onto `workspace` (audit event `isolation_forced`).
...
- `workspaces.provision_env_files(ws_path, canonical, env_files)` — copies manifest-declared gitignored secrets (`delivery.env_files`, e.g. atlas's `.env` with Alpaca keys) from the canonical into the clone right after create_workspace (git clone doesn't carry gitignored files — the 2026-08-10 credentials gap). Never raises; missing sources skipped; returns entries copied (audited on `workspace_created.env_files`). `workspaces.env_file_violation(entry)` — pure belt: refuses absolute/`~`/`..` paths (manifest validation enforces the same); symlink sources refused in the copy step. Wired in `session.run_session` only when the delivery contract declares `env_files`.
...
- `agents.build_subagents(skill_cfg, fallback_model)` — compiles a skill's frontmatter `subagents:` list into SDK `AgentDefinition`s for `ClaudeAgentOptions(agents=...)` (in-session Task-tool delegation). Skips god/host/internal/no_llm/missing skills. `agents.normalize_effort(e)` validates against the SDK ladder (low|medium|high|xhigh|max — `xhigh` is native, not remapped); `agents.skill_to_agent_definition(cfg)` is the pure per-skill mapping — tested in `tests/test_agents.py`.
- `delivery.resolve_delivery_cwd(runtime, delivery, is_deploy)` — pure: dev-repo topology scopes a non-deploy session to the canonical dev repo (the separate git thread); deploy jobs / in-place / content / legacy stay on the runtime clone. `delivery.deploy_permitted(delivery, trigger)` — pure: deploy authority (deployable + autonomy gated-auto|manual-only|human-approval × human/autonomous trigger) → allow|refuse|needs_approval. `delivery.classify_trigger(created_by)`, `delivery.is_deploy_skill(name, delivery)` pure. `delivery.load_project_manifest(slug)` loads the contract, fails open to None. Consumed by `session._resolve_project` (cwd + audit `project_cwd_resolved`) and the run_session deploy gate (raises `DeployRefused`/`DeployNeedsApproval`, handled in `main._process_job`). Tested in `tests/test_delivery.py`. See `docs/superpowers/plans/2026-07-27-project-delivery-segregation.md`.
...
- `session.is_api_terminal_summary(summary)` / `session.usage_is_empty(usage)` / `session.is_api_terminal_session(summary, usage)` — pure classifiers used inside `_run_in_process` to reclassify as failed those sessions whose only "output" is the bundled Claude CLI's Anthropic-side 5xx / timeout / overloaded banner. The two-signal predicate (banner shape AND all-zero usage) fires only for the specific SDK failure mode that hid a 529 as `completed` for job 143c8cfb (2026-08-17, atlas governor silently dark for 10 days); it deliberately does NOT trigger on legitimate summaries that mention "API error" in prose. Raising inside `_run_in_process` engages the existing `_process_job` exception path → `_maybe_escalate` (skill's on_failure retry, then L2 self-diagnose). Tested in `tests/test_api_terminal.py`.
...
- `reconcile.requeue_stranded_queued() -> int` — startup hook (after `reconcile_orphaned_jobs`, before the job loop): re-RPUSHes every `queued` row absent from the Redis list (audit event `queued_requeued`). Pure helper `stranded_queued_ids(rows, redis_ids)`. Added 2026-08-31 (F2.3) alongside the slot-before-BLPOP job-loop change (`_run_with_held_slot`), which stops NEW strandings from happening.
- `session.SkillResolutionError` / `registry.skills.SkillFrontmatterError` — raised when an explicit kind's skill is missing or its frontmatter is corrupt; `main._process_job` handles it terminally (error_category `skill_contract`, no escalation). Added 2026-08-31 (F1.6) — previously such jobs silently ran on registry defaults.
- `mcp_dispatch` — `enqueue_job` rejects `kind='god'` and strips `isolation`/`permission_mode`/`permission` from payloads (`_sanitize_payload`, pure). Added 2026-08-31 (F1.3).
