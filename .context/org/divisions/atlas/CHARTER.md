...
| `alpha-governor` | worker | guarded-writer | Alpha-lab governor (daily 09:30 UTC + idle-queue triggered): resume stalled idea chains (6h staleness, double-dispatch guarded), drain INBOX within caps, budget audit via `alphalab.cli audit`, verdict spot-check, flywheel audits (LESSONS/refinement/scout liveness), AUDIT ledger appends (workspace-isolated, project_slug: atlas) |
| `alpha-scout` | worker | read-only | Flywheel idea generator (08:10 + 20:10 UTC): FAMILIES/LESSONS-fenced, mechanism-first candidate generation + ledger mining, dedup, FILE-mode dispatch within capacity caps; writes nothing (read-only + dispatch) |
| `atlas-quant-validate` | worker | guarded-writer | Weekly validation desk (Sun 14:10 UTC): quantlab stack over the roster — costed backtest, walk-forward worst-fold judgment, 8-check leakage critic, Deflated Sharpe vs lifetime trial N — producing quant/reports/R-#### + health pass; reports only, no order path (workspace-isolated, project_slug: atlas) |
| `atlas-quant-governor` | worker | guarded-writer | Weekly quant-desk audit (Tue 12:20 UTC): validate-row liveness, trials-before-report ordering, independent DSR recompute from metrics.json (mismatch = PROTOCOL-VIOLATION), FLAG acknowledgement, AUDIT ledger entry; frozen evaluator (workspace-isolated, project_slug: atlas) |
