...
| `server-deploy` | worker | prod-operator | dev→prod: ff-only pull, migrate-safe, test gate, restart |
...
- Server code never auto-merges (INV-4); code-review must LGTM (INV-13).
