...
| `pickem` | service | `pickem.chrispiserchia.com` (:8793) | web | native-web | `github.com/alfredbot-ai-butler/pickem` (canonical, private; Mini dev clone `~/Documents/repos/pickem`) | — |
...
`pickem` is the public dashboard for the Arlington Degenerates CBS pick'em
league: a FastAPI + React/Vite SPA over a cookie-authenticated CBS Sports
...
`volumes/pickem/cbs_cookie_header.txt`), and the sqlite db plus the cookie
live under `volumes/pickem/`, outside the repo, so a redeploy never touches
either.
...
| `pickem` | dev-repo (GitHub `alfredbot-ai-butler/pickem` canonical, private — bot-account DR pattern, transferable to the owner; Mini dev clone `~/Documents/repos/pickem`; runtime clone pull-only) | yes · gated-auto | **ACTIVE from creation 2026-09-09** — block written with the project (`env_files: [".env"]`; gates: pytest, `npm run build` when `frontend/**` changes, `/healthz` 200) |
