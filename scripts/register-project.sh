...
# ── Auto-register in PROJECTS_REGISTRY.md (Rec 12) ──────────────────────
REGISTRY="$PROJECT_DIR/.context/PROJECTS_REGISTRY.md"
if [[ -f "$REGISTRY" ]] && ! grep -q "\`$SLUG\`" "$REGISTRY"; then
    PORT_DISPLAY="${PORT:-—}"
    # Insert row before the AUTOGENERATED_END marker
    sed -i '' "/PROJECTS_AUTOGENERATED_END/i\\
| \`$SLUG\` | $TYPE | \`$HOSTNAME\` | — | — | \`github.com/Piserchia/$SLUG\` | — |
" "$REGISTRY"
    echo "  Added $SLUG to PROJECTS_REGISTRY.md"
fi
